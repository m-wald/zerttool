...
class PruefungController extends AbstractActionController {
	...
	private function checkDate() {
		...
		if ($kurs->load($this->pruefung->getKursId())) {
			if ($this->pruefung->getTermin() < $kurs->getKurs_start()) {
			...
			}elseif ($this->pruefung->getTermin() > date_sub(new \DateTime(strtotime($kurs->getKurs_ende())), new \DateInterval("P4D"))) {
				...
				echo "<br>Minus 4 Tage: " .date_sub(new \DateTime(strtotime($kurs->getKurs_ende())), new \DateInterval("P4D"));
